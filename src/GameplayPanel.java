...
public class GameplayPanel extends JPanel {
    ...
    private static final int ROUND_TIME_LIMIT = 10;
    
    private long levelScore = 0; 
    private static final long WIN_POINTS = 100; 
    private static final long FAIL_PENALTY = 65; 
    private static final long MIN_SCORE_TO_UNLOCK = 450; 
    ...
    public void loadLevel(LevelConfig config, int roundCount, User user) {
        ...
        // PERBAIKAN: unlockedLevel adalah level tertinggi yang sudah terbuka
        int currentUnlockedLevel = user.getUnlockedLevel();
        // Level tertinggi = level yang sama dengan unlockedLevel
        boolean isHighestLevel = (config.getId() == currentUnlockedLevel);
        boolean isCompleted = launcher.getUserManager().isLevelCompletedAndLocked(user.getId(), config.getId());
        
        // Debug print
        System.out.println("DEBUG - Current Level: " + config.getId());
        System.out.println("DEBUG - Unlocked Level: " + currentUnlockedLevel);
        System.out.println("DEBUG - Is Highest Level: " + isHighestLevel);
        System.out.println("DEBUG - Is Completed And Locked: " + isCompleted);
        
        // Cek apakah ini level lama yang sudah selesai
        // Level lama tetap bisa dimainkan, tapi score tidak akan di-update
        // Jadi tidak perlu ada validasi khusus di sini, biarkan load seperti biasa
    ...
    private void buildGrid() {
        ...
        System.out.println("Hazard icon width: " + hazardIcon.getIconWidth());
        System.out.println("Hazard icon height: " + hazardIcon.getIconHeight());
        ...
        //Debug Warna Tile Hazard
        try {
            BufferedImage hazardImg = ImageIO.read(new File("assets/tile_hazard.png"));
            hazardIcon = new ImageIcon(hazardImg);
            
            BufferedImage defaultImg = ImageIO.read(new File("assets/tile_default.png"));
            defaultIcon = new ImageIcon(defaultImg);
            
            BufferedImage pressedImg = ImageIO.read(new File("assets/tile_pressed.png"));
            pressedIcon = new ImageIcon(pressedImg);
            
            BufferedImage pointDefaultImg = ImageIO.read(new File("assets/tilepoint_default.png"));
            pointDefaultIcon = new ImageIcon(pointDefaultImg);
            
            BufferedImage pointPressedImg = ImageIO.read(new File("assets/tilepoint_pressed.png"));
            pointPressedIcon = new ImageIcon(pointPressedImg);
        } catch (IOException e) {
            e.printStackTrace();
            System.err.println("Failed to load images!");
        }
        Image img = hazardIcon.getImage();
        if (img instanceof BufferedImage) {
            BufferedImage buffered = (BufferedImage) img;
            int centerX = buffered.getWidth() / 2;
            int centerY = buffered.getHeight() / 2;
            int rgb = buffered.getRGB(centerX, centerY);
            Color pixelColor = new Color(rgb);
            System.out.println("Hazard center pixel RGB: " + 
                pixelColor.getRed() + ", " + 
                pixelColor.getGreen() + ", " + 
                pixelColor.getBlue());
        }
...
private void refreshTiles() {
    ...
    private void handleLevelComplete() {
        ...
        int currentUnlockedLevel = currentUser.getUnlockedLevel();
        ...
        if (currentUser != null) {
            ...
            if (canUnlock && levelConfig.getId() == currentUser.getUnlockedLevel()) {
                int nextLevel = levelConfig.getId() + 1;
                launcher.getUserManager().updateUnlockedLevel(currentUser.getId(), nextLevel);
